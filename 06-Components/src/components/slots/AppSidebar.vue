<template>
  <div class="backdrop" :class="{ visible: isOpen }" @click="onClose" />
  <aside :class="{ visible: isOpen, 'on-right': onRight }">
    <div class="content">
      <slot name="header" />
      <main>
        <slot />
...
<script setup lang="ts">
  defineProps<{ isOpen: boolean; onRight?: boolean; onClose: () => void }>()
</script>
